...
**`0009`, `0010` and `0011` were all pasted on 2026-08-24**, and the three are recorded separately
because the evidence for them is not the same strength.

`0011`'s paste is **verified over the wire**: `calendar_connections` resolves for an authenticated
caller with exactly the four granted columns and answers `42501 permission denied` to `anon`, which
is the grant in `0011` doing what it says. `0010`'s is verified the same way — `chore_exclusions`
answers with its four granted columns, and that assertion was **red by design until the paste**.
`0009`'s is **owner-asserted, not measured**, and that is a property of the migration rather than a
gap in the record:
...
- `0009` changes only two indexes, and `check:live` covers tables, columns, RPCs and Edge Functions
  ...
  "the database matches the repo". Nothing in this repo can confirm or refute that paste; the file
  is `drop index if exists` + `create unique index if not exists` over strictly weaker indexes, so
  a re-paste is a safe no-op and is the only cheap way to make it certain again.

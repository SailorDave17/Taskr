...
**Where it stands: the set is EMPTY, and #171 (2026-09-10) is the first story to add a migration that this check cannot see AT ALL — *measured* **65 of 65** on both sides of `0040`, by construction rather than by luck. `0040` WAS applied in that story's own session; the two instruments that can see it moved (`probe:live-grants` 19 of 20 → 20 of 20, and the read-only catalog query), and this check did not.**
`0040` creates `invitations` and `redeem_invitation`, and **neither joins `liveSchema.js`**: this
check probes what the CLIENT reads and calls, and #171 ships no client code — the reads arrive with
#172 and the call with #173. An entry today reddens `src/lib/liveSchema.test.js`, which holds both

...
(#194) that evening. `0016` (#152) did NOT invert it and is not a twelfth: a migration made only
of an RLS policy has no probe here that could go red, so the set stayed EMPTY on both sides of that
paste and the paste is confirmed by the catalog instead (#198). **Where it stands: the excused-red
set is EMPTY, *measured* at 24 of 24 on 2026-08-26 and *re-measured* at 24 of 24 on 2026-08-27, so
ANY red, on any subject, is real.**

[`docs/access-model.md`](docs/access-model.md) carries the excused-red table — empty as of the
`0015` paste — and
the history of the eleven times that set has been inverted, which is the

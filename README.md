...
can ship it: *measured* **36 of 36** immediately before the name was listed and **36 of 37**
immediately after, in #210's own session, the one red naming exactly that function. Until then the
deployed app's capture flow reports the failure and hands the member the typed field, which is the
fallback #214 later proves in production. `scripts/deploy-function.mjs` carries the name as
PENDING, so a bare deploy does not try to ship a directory that is not there.

...
**EMPTY**. **Later the same day #53 put `0012` (repeating chores) into the repo, and it has NOT been
pasted** — so the run now reads a *measured* **22 of 24** with exactly **TWO** expected reds, the
`chores` columns and the `catch_up_repeats` function, both clearing on that one paste and on
nothing else. Until it lands, every app boot also shows an error strip naming `catching up
repeats` — the gap is visible in the product, not only in the check. Any red on any *other* subject
is real.

[`docs/access-model.md`](docs/access-model.md) carries the excused-red table — the two `0012` rows,
as of #53's merge — and the history of the seven times that set has been inverted, which is the

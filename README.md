...
**Where it stands: the set holds ONE row, opened by #210 on 2026-09-04 and cleared by nothing
short of #209's deploy.** `extract-description` is the Edge Function the plain-language capacity
flow invokes (`src/lib/capture.js`), named in `LIVE_EDGE_FUNCTIONS` ahead of #208 writing it — owner
decision at #210's pickup — so its probe answers **NOT DEPLOYED** until `npm run deploy:function`
ships it: *measured* **36 of 36** immediately before the name was listed and **36 of 37**
...
`scripts/deploy-function.mjs` now carries the name as DEPLOYABLE rather than PENDING and a bare
deploy ships it; what #209 adds is the deploy itself and the `ANTHROPIC_API_KEY` function secret
the function refuses by name without. `0036` (its call ledger, which no client reads) was applied
...
**19 of 19, every table control row agreeing** after. Until the deploy, the deployed app's capture flow reports
the failure and hands the member the typed field, which is the fallback #214 later proves in
production.
...
[`docs/access-model.md`](docs/access-model.md) carries the excused-red table — one row since #210,
`extract-description`, after #96's two (`calendar_busy` and `calendar-busy`) drained the same day;

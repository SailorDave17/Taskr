...
- **#430 opened THREE rows and #431 TWO on 2026-09-11, and neither story drained its rows in its
  ...
  **Not measured:** no `check:live` run was taken in either session, so these are the reds the
  listings predict, not reds observed. Recorded in README's `check:live` cell in the same changes.
...
- **The server half is the `leave-household` Edge Function**, because the refresh token behind a
  ...
  If the token read fails, nothing changes and the person is asked to try again. If the sign-in
  cannot be deleted, the person is told as a warning, because they have already left.
...
- **The re-deal.** It runs in the browser before the leave, with the leaver left out
  ...
  the leave itself and dealt at the next capacity change. A removal now re-deals afterwards too.

...
export async function createHousehold(name) {
  ...
  return unwrap(
    await getSupabase().rpc('create_household', { household_name: trimmed }),
...
export async function listMembers() {
  return (
    unwrap(
      await getSupabase().from('members').select('*').order('created_at', { ascending: true }),
      'loading the roster',
    ) ?? []
  )
}
...
export async function updateMember(id, { displayName, weeklyMinutes }) {
  ...
  return unwrap(
    await getSupabase().from('members').update(patch).eq('id', id).select().single(),
    'saving the change',
  )
}
